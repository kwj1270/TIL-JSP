...
종료 구문
```
## 5.3. Long VARCHAR/text 타입 값 지정하기
...
***  
# 7. JDBC에서 트랜잭션 처리
자바에서도 트랜잭션 처리를 할 수 있다.     
중요한 것은 DML 명령어에서만 사용가능하다.  
```
SELECT, INSERT, UPDATE, DELETE
```

JDBC에서 트랜잭션을 구현하는 방법에는 2가지가 있다.    
* JDBC의 오토 커밋 모드를 false로 지정하는 방법    
* JTA(Java Transaction API)를 이용하는 방법   
  
단일 데이터베이스에 접근하는 경우 JDBC 기반의 트랜잭션을 이용해서 구현하고,      
두 개 이상의 데이터베이스를 트랜잭션으로 처리하려면 JTA를 이용해야 한다.    
JTA는 나중에 살펴보도록 하고 JDBC API에 대한 내용을 살펴보도록 하자.    
   
**간단 구조**
```
try {
  ...
  conn.setAutoCommit(false); // 오토 커밋을 false로 설정해서 수동 커밋 모드로 사용한다.  
  ...
  쿼리 실행
  ...
  conn.commit(); // 쿼리가 에러가 발생하지 않고 쭉 절차대로 실행되면    
} catch(SQLException ex){
    if(conn != null){
      conn.rollback();  // 정상적으로 연결은 됬는데 에러가 발생했다면 롤백을 해라
    }
}
